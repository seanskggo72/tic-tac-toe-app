...
const Check_state = (gd) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (gd[0][i][1] === true && gd[1][i][1] === true && gd[2][i][1] === true) return true;
    if (gd[0][i][1] === false && gd[1][i][1] === false && gd[2][i][1] === false) return true;
  }
  // check columns
  if (gd[0][0][1] === true && gd[1][1][1] === true && gd[2][2][1] === true) return true;
  if (gd[0][0][1] === false && gd[1][1][1] === false && gd[2][2][1] === false) return true;
  if (gd[0][2][1] === true && gd[1][1][1] === true && gd[2][0][1] === true) return true;
  if (gd[0][2][1] === false && gd[1][1][1] === false && gd[2][0][1] === false) return true;
  // Check draw
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      if (gd[i][j][1] === null) return false;
    }
  }
  return true;
}
